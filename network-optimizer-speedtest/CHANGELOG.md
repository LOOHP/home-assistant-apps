...
Fourth preview of v2.9.0. See the [v2.9.0-preview3 notes](https://github.com/Ozark-Connect/NetworkOptimizer/releases/tag/v2.9.0-preview3) for what came before.
...
- **A learned profile shapes from what the line measured** - in the first preview it was still applied against the plan speed you entered, so the ceiling never quite matched the line. The peak the week measured is the ceiling now, the "use learned peaks as nominal" option is gone, and a gateway sample that comes back at zero counts as a failed one.
